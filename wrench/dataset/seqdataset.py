...
def build_bert_embeddings(corpus, bert_model_name, device):
    ...
    # Chia câu nếu dài hơn 510 token BERT
        ...
        if len_bert_tokens >= 510:
            ...
            while (np.asarray(bert_length_list) >= 510).any():
                ...
                for tokens, bert_len in zip(sents_list, bert_length_list):
                    ...
                    sent_lens = [len(word_tokenize(st)) for st in splitted_sents]
    ...
    embs = []
    for i, sent in enumerate(tqdm(standarized_sents, desc='Extracting BERT embeddings...')):
        joint_sent = ' '.join(sent)
        bert_tokens = tokenizer(joint_sent, return_tensors="pt", truncation=True, padding=True, max_length=512)
        input_ids = bert_tokens["input_ids"].to(device)

        # Tính embedding từ BERT
        with torch.no_grad():
            outputs = model(input_ids)
            last_hidden_states = outputs.last_hidden_state.squeeze(0).cpu()

        # Bỏ token đặc biệt ([CLS], [SEP])
        trunc_hidden_states = last_hidden_states[1:-1, :]

        # Căn chỉnh token giữa văn bản gốc và token BERT
        word_tokens = word_tokenize(joint_sent)
        alignments = []
        token_idx = 0

        for word in word_tokens:
            aligned_idxs = []
            while token_idx < len(trunc_hidden_states):
                if word.lower() in tokenizer.convert_ids_to_tokens(input_ids[0][token_idx]).lower():
                    aligned_idxs.append(token_idx)
                    token_idx += 1
                else:
                    break
            alignments.append(aligned_idxs)

        # Lấy embedding trung bình của các token ánh xạ vào một từ
        emb_list = []
        for j, idx in enumerate(alignments):
            if not idx:
                idx = [0] if j == 0 else [max(alignments[j - 1])]
            emb_list.append(trunc_hidden_states[idx, :].mean(dim=0))

        bert_emb = torch.stack(emb_list)
        embs.append(bert_emb.cpu().detach())

    # Gộp embedding theo câu gốc
    combined_embs = []
    for o2n in o2n_map:
        if len(o2n) == 1:
